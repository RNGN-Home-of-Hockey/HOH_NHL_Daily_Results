#!/usr/bin/env python3
"""Collect additive NHL team reports at per-game granularity for two seasons.

These rows complement HOH play-by-play features with official special-teams,
score-state and faceoff splits that are useful for H2H analysis.
"""
...
UA="HOH-NHL-Team-Game-Reports/1.0"
...
def fetch_report(task,timeout,attempts,fetched_at):
    ...
    return {
        "season":season,"game_type":game_type,"report":report,
        "url":url,"api_rows":len(data),"matched_rows":len(rows),"unmatched_rows":unmatched,
        "rows":rows,
    }
...
def value_sql(row):
    vals=[
        row["season_id"],row["game_type"],row["report_name"],row["game_pk"],
...
def write_sql(out_dir,rows,fetched_at):
...
def main():
    ...
    with cf.ThreadPoolExecutor(max_workers=workers) as pool:
        fm={pool.submit(fetch_report,t,args.timeout,args.attempts,fetched_at):t for t in tasks}
    ...
    files=write_sql(out,rows,fetched_at)
    summary={
