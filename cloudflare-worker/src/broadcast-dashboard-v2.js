...
function browserApp(){
const $=s=>document.querySelector(s);let games=[],selected=null,currentCards=[],historicalCards=[],liveCards=[],liveTimer=null,currentData=null;
...
<div class="drawerback" id="drawer"><div class="drawer"><div class="dtop"><div class="dtitle">ПРЕДПРОСМОТР КАРТОЧКИ</div><button class="close" id="close">Закрыть</button></div><div class="preview" id="previewcard"></div><div class="dfoot">Компактный эфирный формат. Длинные пояснения не выводятся. Кнопка «ПОКАЗАТЬ» подключим к защищённому overlay следующим шагом.</div></div></div>
