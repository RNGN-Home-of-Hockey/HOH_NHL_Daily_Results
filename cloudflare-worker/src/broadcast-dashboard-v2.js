...
function renderCombinedCards(){const seen=new Set();const merged=[];for(const c of [...liveCards,...historicalCards]){const k=c.id||`${c.type}:${c.market?.type||''}:${c.market?.subject||''}`;if(seen.has(k))continue;seen.add(k);merged.push(c)}merged.sort((a,b)=>{const live=Number(b?.kind==='live')-Number(a?.kind==='live');if(live)return live;return airScore(b)-airScore(a)});renderCards(merged.slice(0,24));syncQueueSummaryFromCards(selected,merged);const priced=merged.filter(hasRealWinlinePrice).length,sub=document.querySelector('.psub'),cov=currentData?.market_coverage;const covText=cov&&Number(cov.provider_exact_market_count)>0?` · покрыто рынков: ${Number(cov.covered_exact_market_count||0)}/${Number(cov.provider_exact_market_count||0)} (${cov.exact_market_coverage_pct??0}%) · 2+ сигнала: ${Number(cov.multi_signal_market_count||0)}`:'';if(sub)sub.textContent=`Сигналов: ${merged.length} · точных линий Winline: ${priced}${covText} · очередь по AIR SCORE`}
