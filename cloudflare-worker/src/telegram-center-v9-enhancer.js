...
async function localizeVisiblePlayers(){const map=await loadRuNames();document.querySelectorAll('[data-player]').forEach(row=>{const id=String(row.dataset.player||'');const name=map[id];if(!name)return;const b=row.querySelector('.rowText b');if(!b||b.dataset.ruDone)return;const badge=b.querySelector('.orangeText')?.outerHTML||'';const original=(b.textContent||'').replace(/^\s*(?:RU|[A-Z]{2,3}|[\p{Extended_Pictographic}\uFE0F\u200D]+)\s+/u,'').trim();b.innerHTML=(badge?badge+' ':'')+esc(name)+(original&&!original.includes(name)?' <span class="v24OriginalName">('+esc(original)+')</span>':'');b.dataset.ruDone='1'})}
async function decoratePlayerSalaries(){const map=await loadSalaries();document.querySelectorAll('.playerRow[data-player]').forEach(row=>{if(row.querySelector('.salaryLine')||row.querySelector('.playerSortValue'))return;const id=String(row.dataset.player||''),s=map[id],box=row.querySelector('.rowText');if(!s||!box)return;const value=Number(s.aav||s.cap_hit||0);if(!value)return;const line=document.createElement('span');line.className='salaryLine';line.textContent='💰 '+money(value)+' / год';box.appendChild(line)})}
...
function tabsVisible(){const tabs=document.querySelector('.tabs');if(!tabs)return false;const cs=getComputedStyle(tabs);return cs.display!=='none'&&cs.visibility!=='hidden'}
function homeMediaAllowed(){return currentTab()==='games'&&tabsVisible()}
function purgeMediaOffHome(){if(homeMediaAllowed())return;document.querySelectorAll('#view .mediaShelf,#view .v12Media,#view .v15Media').forEach(x=>x.remove());mediaLoaded=false}
