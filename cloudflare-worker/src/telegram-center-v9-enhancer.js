...
export async function handleTelegramCenterV9Enhancer(request, env, path) {
  ...
  // Shadow the V7 schedule route with a date-safe implementation. NHL /schedule
  // returns a week; some nested game objects do not carry gameDate themselves.
  if (path === "/api/telegram-center-v7/games" && request.method === "GET") {
    return gamesForDate(request, env);
  }
...
function mediaConfig(env){
  ...
  const items=raw.filter(x=>String(x.url||"").trim()).map(x=>({
    kind:x.kind,url:String(x.url).trim(),title:String(x.title||defaultMediaTitle(x.kind)).trim(),
    thumb:String(x.thumb||youtubeThumb(x.url)||"").trim(),
  }));
  return json({ok:true,configured:items.length===3,items});
}
function defaultMediaTitle(kind){return kind==="news"?"Свежий HOME OF HOCKEY NEWS":"Свежий вертикальный ролик"}
function youtubeThumb(url){const m=String(url||"").match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?v=|shorts\/))([A-Za-z0-9_-]{6,})/);return m?`https://i.ytimg.com/vi/${m[1]}/hqdefault.jpg`:""}
...
.playerRow,.myRow,.teamRow{border-radius:18px!important;padding:12px 13px!important;min-height:74px}.playerRow img,.myRow img,.teamRow img{width:50px!important;height:50px!important;border-radius:13px!important}.rowText b{font-size:15px!important}.rowText small{font-size:10px!important;margin-top:3px;display:block}.myRow .rowText b{font-size:16px!important}
...
let ruNames=null,mediaLoaded=false,calendarBusy=false;
...
function run(){decorateMetrics();installCalendarButton();localizeVisiblePlayers();fixMineNames();injectMedia()}
