...
async function openSettings(){
  ...
  '<div class="v22Field"><label>Тема</label><select id="v22_theme_mode"><option value="dark" '+(p.theme_mode!=='light'?'selected':'')+'>Тёмная</option><option value="light" '+(p.theme_mode==='light'?'selected':'')+'>Светлая</option></select></div>'+
  ...
  document.getElementById('v22Save').onclick=async()=>{const btn=document.getElementById('v22Save');btn.disabled=true;try{
    ...
    lastProfile=await H.api(API+'/me',{method:'PUT',body:JSON.stringify(body)});H.setTheme?.(body.theme_mode||'dark');window.HOHSetNoSpoilers?.(body.no_spoilers);alert('Профиль сохранён');
...
me(true).then(d=>{const p=d?.profile;if(!p?.exists)return;if(p.theme_mode&&H.theme?.()!==p.theme_mode)H.setTheme?.(p.theme_mode);window.HOHSetNoSpoilers?.(Boolean(p.no_spoilers))}).catch(()=>{});
