...
async function players(request,env){
  ...
  for(const [idRaw,p] of Object.entries(pronunciationCache?.players||{})){
    ...
    map.set(id,{...prev,player_id:id,full_name_en:prev.full_name_en||String(p?.full_name_en||"").trim(),full_name_ru:prev.full_name_ru||fullNames?.[idRaw]||fallbackRuName(prev.full_name_en||String(p?.full_name_en||""))||null,current_team_tri:up(prev.current_team_tri||p?.team_tri||"")});
  }
...
function fallbackRuName(value){
  const words=String(value||"").trim().split(/\s+/).filter(Boolean);
  return words.map(word=>{
    let s=word.toLowerCase();
    for(const [a,b] of [["shch","щ"],["yo","ё"],["yu","ю"],["ya","я"],["zh","ж"],["kh","х"],["ts","ц"],["ch","ч"],["sh","ш"],["ph","ф"],["th","т"],["ck","к"],["qu","кв"],["mc","мак"]])s=s.split(a).join(b);
    const m={a:"а",b:"б",c:"к",d:"д",e:"е",f:"ф",g:"г",h:"х",i:"и",j:"дж",k:"к",l:"л",m:"м",n:"н",o:"о",p:"п",q:"к",r:"р",s:"с",t:"т",u:"у",v:"в",w:"в",x:"кс",y:"й",z:"з"};
    s=[...s].map(ch=>m[ch]||ch).join("");
