...
async function matchupApi(request,env,gamePk){
  try{
    ...
    const [markets,snapshotsR,playersR]=await Promise.all([
      evaluateMarketLines(env.DB,game,{window}),
      env.DB.prepare(`
    ...
    const snapshots=groupBy(snapshotsR.results||[],r=>r.team_tri);
    const players=groupBy(playersR.results||[],r=>r.current_team_tri);
    ...
    return json({
      ...
      methodology:{
        ...
        confidence:"distance from 50%; deterministic context, not a probability forecast",
      },
...
function browserApp(){
  const $=s=>document.querySelector(s);const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const pct=v=>Number.isFinite(Number(v))?(Number(v)*100).toFixed(0)+'%':'—';const num=(v,d=1)=>Number.isFinite(Number(v))?Number(v).toFixed(d):'—';
  ...
  function render(d){const g=d.game;const away=d.teams[g.away_tri]||{},home=d.teams[g.home_tri]||{};$('#content').innerHTML=`<section class="hero"><div class="club">${g.away_logo?`<img src="${esc(g.away_logo)}">`:''}<b>${esc(g.away_tri)}</b></div><div><div class="eyebrow">MATCHUP · ${esc(g.game_state||'PREGAME')}</div><h1>${esc(g.away_tri)} — ${esc(g.home_tri)}</h1><span>${esc(g.start_utc||'')}</span></div><div class="club right">${g.home_logo?`<img src="${esc(g.home_logo)}">`:''}<b>${esc(g.home_tri)}</b></div></section><div class="seg">${[5,10,20].map(w=>`<button data-w="${w}" class="${w===windowGames?'active':''}">${w}</button>`).join('')}</div><section class="grid"><article class="panel"><h3>Точные линии · top</h3><div class="markets">${(d.top_markets||[]).map(m=>`<div class="market"><span><b>${esc(m.label)}</b><small>${esc(m.market_type)} · sample ${m.sample}</small></span><strong>${pct(m.combined_rate)}</strong><i>${num(m.confidence,0)} conf</i></div>`).join('')||'<div class="empty small">Нет сильных сигналов</div>'}</div></article><article class="panel"><h3>${esc(g.away_tri)} · 20 матчей</h3>${teamBlock(away,g.away_tri)}<h3>${esc(g.home_tri)} · 20 матчей</h3>${teamBlock(home,g.home_tri)}</article></section><section class="panel"><h3>Все рынки</h3><div class="allMarkets">${(d.markets||[]).map(m=>`<div><span>${esc(m.label)}</span><b>${pct(m.combined_rate)}</b><small>${Object.entries(m.evidence?.components||{}).map(([k,v])=>esc(k)+' '+pct(v)).join(' · ')}</small></div>`).join('')}</div></section>`;document.querySelectorAll('[data-w]').forEach(b=>b.onclick=()=>{windowGames=Number(b.dataset.w);load()})}
