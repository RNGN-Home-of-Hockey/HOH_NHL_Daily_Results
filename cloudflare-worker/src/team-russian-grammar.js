...
function fixSubject(text,subject,team){
  ...
  const firstRe=new RegExp(`(${escaped}\\s+)(пропускал)\\s+(первым)(?=\\s|$|[.,;:!?])`,"gi");
  ...
  for(const [word,forms] of Object.entries(WORD_FORMS)){
    ...
    const patterns=[
      new RegExp(`(${escaped}\\s+)(${word})(?=\\s|$|[.,;:!?])`,"gi"),
      new RegExp(`(${escaped}\\s+не\\s+)(${word})(?=\\s|$|[.,;:!?])`,"gi"),
      new RegExp(`(${escaped}\\s+(?:дома|в\\s+гостях)\\s+)(${word})(?=\\s|$|[.,;:!?])`,"gi"),
    ];
