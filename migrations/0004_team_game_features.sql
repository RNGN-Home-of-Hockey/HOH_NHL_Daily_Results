...
CREATE TABLE IF NOT EXISTS team_game_features (
  ...
  final_goals_for INTEGER NOT NULL,
  final_goals_against INTEGER NOT NULL,
  total_goals INTEGER NOT NULL,
  final_goal_diff INTEGER NOT NULL,
  final_win INTEGER NOT NULL CHECK (final_win IN (0, 1)),

  regulation_goals_for INTEGER NOT NULL,
  regulation_goals_against INTEGER NOT NULL,
  regulation_goal_diff INTEGER NOT NULL,
  regulation_result TEXT NOT NULL CHECK (regulation_result IN ('W', 'L', 'T')),
  went_ot INTEGER NOT NULL DEFAULT 0 CHECK (went_ot IN (0, 1)),
  went_so INTEGER NOT NULL DEFAULT 0 CHECK (went_so IN (0, 1)),

  p1_goals_for INTEGER NOT NULL DEFAULT 0,
  p1_goals_against INTEGER NOT NULL DEFAULT 0,
  p2_goals_for INTEGER NOT NULL DEFAULT 0,
  p2_goals_against INTEGER NOT NULL DEFAULT 0,
  p3_goals_for INTEGER NOT NULL DEFAULT 0,
  p3_goals_against INTEGER NOT NULL DEFAULT 0,
  score_after_p1_diff INTEGER NOT NULL DEFAULT 0,
  score_after_p2_diff INTEGER NOT NULL DEFAULT 0,
  first_goal_for INTEGER CHECK (first_goal_for IN (0, 1) OR first_goal_for IS NULL),
  ...
  p1_shots_for INTEGER,
  p1_shots_against INTEGER,
  p2_shots_for INTEGER,
  p2_shots_against INTEGER,
  p3_shots_for INTEGER,
  p3_shots_against INTEGER,
  ...
  previous_game_utc TEXT,
  rest_days INTEGER,
  is_back_to_back INTEGER NOT NULL DEFAULT 0 CHECK (is_back_to_back IN (0, 1)),
...
CREATE INDEX IF NOT EXISTS idx_team_game_features_team_time
  ON team_game_features(team_tri, scheduled_start_utc DESC);

CREATE INDEX IF NOT EXISTS idx_team_game_features_season_team
  ON team_game_features(season_id, team_tri, scheduled_start_utc DESC);

CREATE INDEX IF NOT EXISTS idx_team_game_features_matchup
  ON team_game_features(team_tri, opponent_tri, scheduled_start_utc DESC);

CREATE INDEX IF NOT EXISTS idx_team_game_features_rest
  ON team_game_features(team_tri, is_back_to_back, scheduled_start_utc DESC);

-- Seed the feature layer for every eligible game already present in Data Core.
WITH
period_agg AS (
  SELECT
    game_pk,
    SUM(CASE WHEN period_number = 1 THEN home_goals ELSE 0 END) AS p1_home,
    SUM(CASE WHEN period_number = 1 THEN away_goals ELSE 0 END) AS p1_away,
    SUM(CASE WHEN period_number = 2 THEN home_goals ELSE 0 END) AS p2_home,
    SUM(CASE WHEN period_number = 2 THEN away_goals ELSE 0 END) AS p2_away,
    SUM(CASE WHEN period_number = 3 THEN home_goals ELSE 0 END) AS p3_home,
    SUM(CASE WHEN period_number = 3 THEN away_goals ELSE 0 END) AS p3_away,
    MAX(CASE WHEN period_type = 'OT' OR period_number > 3 THEN 1 ELSE 0 END) AS went_ot,
    MAX(CASE WHEN period_type = 'SO' THEN 1 ELSE 0 END) AS went_so
  FROM period_scores
  GROUP BY game_pk
),
first_goal AS (
  ...
  FROM (
    SELECT game_pk, team_tri,
           ROW_NUMBER() OVER (PARTITION BY game_pk ORDER BY sort_order) AS rn
    FROM game_events
    WHERE event_type = 'goal' AND team_tri IS NOT NULL
  ) ranked
  WHERE rn = 1
),
event_agg AS (
  SELECT
    g.game_pk,
    SUM(CASE WHEN ge.event_type IN ('goal','shot-on-goal','missed-shot') AND ge.team_tri = g.home_tri THEN 1
             WHEN ge.event_type = 'blocked-shot' AND ge.team_tri = g.away_tri THEN 1 ELSE 0 END) AS home_corsi,
    SUM(CASE WHEN ge.event_type IN ('goal','shot-on-goal','missed-shot') AND ge.team_tri = g.away_tri THEN 1
             WHEN ge.event_type = 'blocked-shot' AND ge.team_tri = g.home_tri THEN 1 ELSE 0 END) AS away_corsi,
    SUM(CASE WHEN ge.event_type IN ('goal','shot-on-goal','missed-shot') AND ge.team_tri = g.home_tri THEN 1 ELSE 0 END) AS home_fenwick,
    SUM(CASE WHEN ge.event_type IN ('goal','shot-on-goal','missed-shot') AND ge.team_tri = g.away_tri THEN 1 ELSE 0 END) AS away_fenwick,
    SUM(CASE WHEN ge.period_number = 1 AND ge.event_type IN ('goal','shot-on-goal') AND ge.team_tri = g.home_tri THEN 1 ELSE 0 END) AS p1_home_shots,
    SUM(CASE WHEN ge.period_number = 1 AND ge.event_type IN ('goal','shot-on-goal') AND ge.team_tri = g.away_tri THEN 1 ELSE 0 END) AS p1_away_shots,
    SUM(CASE WHEN ge.period_number = 2 AND ge.event_type IN ('goal','shot-on-goal') AND ge.team_tri = g.home_tri THEN 1 ELSE 0 END) AS p2_home_shots,
    SUM(CASE WHEN ge.period_number = 2 AND ge.event_type IN ('goal','shot-on-goal') AND ge.team_tri = g.away_tri THEN 1 ELSE 0 END) AS p2_away_shots,
    SUM(CASE WHEN ge.period_number = 3 AND ge.event_type IN ('goal','shot-on-goal') AND ge.team_tri = g.home_tri THEN 1 ELSE 0 END) AS p3_home_shots,
    SUM(CASE WHEN ge.period_number = 3 AND ge.event_type IN ('goal','shot-on-goal') AND ge.team_tri = g.away_tri THEN 1 ELSE 0 END) AS p3_away_shots
  FROM games g
  LEFT JOIN game_events ge ON ge.game_pk = g.game_pk
  WHERE g.game_type IN (2,3)
  GROUP BY g.game_pk
),
base AS (
  SELECT
    g.game_pk, g.home_tri AS team_tri, g.away_tri AS opponent_tri,
    g.season_id, g.game_type, g.scheduled_start_utc, 1 AS is_home,
    g.home_score AS final_goals_for, g.away_score AS final_goals_against,
    ...
    CASE WHEN fg.team_tri IS NULL THEN NULL WHEN fg.team_tri = g.home_tri THEN 1 ELSE 0 END AS first_goal_for,
  ...
  FROM games g
  LEFT JOIN period_agg p ON p.game_pk = g.game_pk
  LEFT JOIN first_goal fg ON fg.game_pk = g.game_pk
  LEFT JOIN event_agg e ON e.game_pk = g.game_pk
  LEFT JOIN team_game_stats hs ON hs.game_pk = g.game_pk AND hs.team_tri = g.home_tri
  LEFT JOIN team_game_stats aws ON aws.game_pk = g.game_pk AND aws.team_tri = g.away_tri
  WHERE g.game_type IN (2,3)

  UNION ALL

  SELECT
    g.game_pk, g.away_tri AS team_tri, g.home_tri AS opponent_tri,
    g.season_id, g.game_type, g.scheduled_start_utc, 0 AS is_home,
    g.away_score AS final_goals_for, g.home_score AS final_goals_against,
    ...
    COALESCE(p.went_ot,0) AS went_ot, COALESCE(p.went_so,0) AS went_so,
    CASE WHEN fg.team_tri IS NULL THEN NULL WHEN fg.team_tri = g.away_tri THEN 1 ELSE 0 END AS first_goal_for,
...
with_prev AS (
  SELECT b.*,
         LAG(scheduled_start_utc) OVER (PARTITION BY team_tri ORDER BY scheduled_start_utc, game_pk) AS previous_game_utc
  FROM base b
)
